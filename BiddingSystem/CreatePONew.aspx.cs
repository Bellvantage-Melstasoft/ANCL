...
{
    public partial class CreatePONew : System.Web.UI.Page
    {
        #region properties
      //  static int PrId = 0;
       // static int BidId = 0;
        ...
        private void LoadGV()
        {
            try
            {
                ...
                if (ViewState["PurchaseType"].ToString() == "1") {
                    ...
                    gvItems.Columns[29].Visible = false;
            ...
            for (int i = 0; i < gvItems.Rows.Count; i++)
            {
                if (((gvItems.Rows[i].FindControl("CheckBox1") as CheckBox).Checked))
                {
                    if (int.Parse(gvItems.Rows[i].Cells[4].Text) != supplierId)
                    {
                        ...
                        PoDetail.ItemPrice = Math.Round(decimal.Parse(gvItems.Rows[i].Cells[18].Text), 4);
                        PoDetail.Quantity = decimal.Parse(gvItems.Rows[i].Cells[15].Text);
                        PoDetail.VatAmount = Math.Round(decimal.Parse(gvItems.Rows[i].Cells[21].Text), 4);
                        PoDetail.NbtAmount = Math.Round(decimal.Parse(gvItems.Rows[i].Cells[20].Text), 4);
                        PoDetail.TotalAmount = Math.Round(decimal.Parse(gvItems.Rows[i].Cells[22].Text), 4);
                        ...
                        PoDetail.HasNbt = int.Parse(gvItems.Rows[i].Cells[24].Text);
                        PoDetail.NbtCalculationType = int.Parse(gvItems.Rows[i].Cells[25].Text);
                        PoDetail.HasVat = int.Parse(gvItems.Rows[i].Cells[23].Text);
                        PoDetail.MeasurementId = int.Parse(gvItems.Rows[i].Cells[26].Text);

...
{
    public partial class CompanyViewRejectedPR : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                try
                {
                    pr_Master = pr_MasterController.FetchRejectedPr(int.Parse(CompanyId));
        ...
        protected void OnRowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    {
                        pr_Details = pr_DetailController.FetchDetailsRejectedPR(item.PrId,item.DepartmentId).ToList();
        ...
        protected void lnkBtnEdit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string save = pr_MasterController.SavePRMasterAtCloning(int.Parse(CompanyId), pr_MasterObj.DateOfRequest, pr_MasterObj.QuotationFor, pr_MasterObj.OurReference, pr_MasterObj.RequestedBy, pr_MasterObj.CreatedDateTime, pr_MasterObj.CreatedBy, pr_MasterObj.UpdatedDateTime, pr_MasterObj.UpdatedBy, 1,0, pr_MasterObj.PrIsApprovedOrRejectedBy, pr_MasterObj.PrIsApprovedOrRejectForBidDate, pr_MasterObj.PrIsApprovedForBid, pr_MasterObj.PrIsApprovedOrRejectedBy, pr_MasterObj.PrIsApprovedOrRejectForBidDate, int.Parse(basePr), pr_MasterObj.PrTypeid, pr_MasterObj.expenseType, pr_MasterObj.Ref01, pr_MasterObj.Ref02, pr_MasterObj.Ref03, pr_MasterObj.Ref04, pr_MasterObj.Ref05, pr_MasterObj.Ref06, pr_MasterObj.PrProcedure, pr_MasterObj.PurchaseType, pr_MasterObj.RequiredDate,pr_MasterObj.MRNRefNumber);
                ...
                if (save !="")
                {
                    ...
                    {
                        pr_DetailController.SavePRDetails(NewPrId, item.ItemId, item.Unit, item.ItemDescription, item.ItemUpdatedBy, item.ItemUpdatedDateTime, item.IsActive, item.Replacement, item.ItemQuantity, item.Purpose, item.EstimatedAmount, item.SampleProvided,item.Remarks,item.MeasurmentId);
                    }
                    ...
                    lblMsg.Text = "Sucessfully created a new PR. Pr Code : " +  save;
                }

...
{
    public partial class CompanyMonitorBids : System.Web.UI.Page
    {
       ...
       // static List<Bidding> Bids;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Session["CompanyId"] != null && Session["UserId"].ToString() != "")
            {
                ...
                ((BiddingAdmin)Page.Master).subTabValue = "CompanyMonitorBids.aspx";
                ((BiddingAdmin)Page.Master).subTabId = "inProgressBidLink";

               // UserId = Session["UserId"].ToString();
              //  CompanyId = int.Parse(Session["CompanyId"].ToString());
                CompanyLogin companyLogin = companyLoginController.GetUserbyuserId(int.Parse(Session["UserId"].ToString()));

                if ((!companyUserAccessController.isAvilableAccess(int.Parse(Session["UserId"].ToString()), int.Parse(Session["CompanyId"].ToString()), 6, 3) && companyLogin.Usertype != "S") && companyLogin.Usertype != "GA")
                {
        ...
        private void GVBind()
        {
            try
            {
              var  Bids = biddingController.GetInProgressBids(int.Parse(Session["CompanyId"].ToString()));
                ...
                foreach (var item in Bids)
                {
                    ...
                    //PrMasterV2 ob = pr_MasterController.getPRMasterDetailByPrId(item.PrId);
                    //item.MRNRefNumber = ob.MrnId.ToString();
                    //if (item.MRNRefNumber != null || item.MRNRefNumber != "")
                    //{
                    //    item.subDepartmentName = MrnMasterController.GetMRNMasterByMrnId(Convert.ToInt32(item.MRNRefNumber)).SubDepartmentName;
                    //}
                }

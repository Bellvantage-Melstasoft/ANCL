...
{
    public partial class RecommendQuotationsItemWise : System.Web.UI.Page
    {
        #region properties
       // static int PrId = 0;
      //  static int BidId = 0;
      ...
      //  static PrMasterV2 PrMaster;
       ...
       // static TabulationMaster tabulationMaster;
            ...
            if (e.Row.RowType == DataControlRowType.DataRow) {
               ...
               // lblNbt.Text = tabulationMaster.NbtAmount.ToString("N2");
                ...
                if (tabulationMaster.RecommendationWasOveridden == 1) {
                    ...
                    lblTabOverridden.Text = "Tabulation Recommendation was Overridden by " + tabulationMaster.RecommendationOveriddenByName + " on " + tabulationMaster.RecommendationOveriddenOn.ToString("dd MMMM yyyy") + " ";
        ...
        private void LoadRejectedBidItems() {
            //var PrMaster = new JavaScriptSerializer().Deserialize<PR_Master>(ViewState["PrMaster"].ToString());
            var RejPrMaster = pr_MasterController.GetPrForQuotationApprovalRej(int.Parse(ViewState["PrId"].ToString()), int.Parse(Session["CompanyId"].ToString()), int.Parse(Session["UserId"].ToString()), int.Parse(Session["DesignationId"].ToString()));
        ...
        protected void btnHold_Click(object sender, EventArgs e) {
            ...
            int result = tabulationRecommendationController.HoldRecommendation(int.Parse(hdnTabulationId.Value), int.Parse(hdnRecommendationId.Value), remark, int.Parse(Session["UserId"].ToString()));
            if (result > 0) {
                ScriptManager.RegisterClientScriptBlock(Updatepanel1, this.Updatepanel1.GetType(), "none", "<script>    $(document).ready(function () { swal({ type: 'success',title: 'SUCCESS',text:'Your work has been saved', showConfirmButton: false,timer: 1500}).then((result) => { window.location = 'ViewPrForQuotationRecommendation.aspx' });; });   </script>", false);
        ...
        protected void btnPrint1_Click(object sender, EventArgs e) {
           ...
           // }
            ...
            Response.AddHeader("content-disposition", "attachment;filename=" + FileName);
            string headerTable = @"<Table><tr><td  colspan='14'><center><h4>Quotation Tabulation For Bid No: B" + bid.BidCode + " </h4></center> </td></tr></Table> <hr>";
            Response.Write(headerTable);
            Response.Charset = "";
            Response.ContentType = "application/vnd.ms-excel";
            using (StringWriter sw = new StringWriter()) {
                ...
                for (int i = 0; i < gv.Rows.Count; i++) {
                    GridViewRow currentRow = gv.Rows[i];
                    if (currentRow.Cells[12].Text == "Selected") {

...
namespace BiddingSystem {
    public partial class ApproveQuotationsNew : System.Web.UI.Page {
        #region properties
        // static int PrId = 0;
        // static int BidId = 0;
        // static int UserId = 0;
            ...
            if (!IsPostBack) {
                if (int.Parse(Session["UserId"].ToString()) != 0) {
                    try {
                        ...
                        ViewState["PrMaster"] = ViewState["PrMaster"] = new JavaScriptSerializer().Serialize(PrMaster);
        ...
        protected void btnViewAttachments_Click(object sender, EventArgs e) {
            ...
            gvDocs.DataSource = new JavaScriptSerializer().Deserialize<PrMasterV2>(ViewState["PrMaster"].ToString()).Bids.Find(b => b.BidId == BidId).SupplierQuotations.Find(q => q.QuotationId == int.Parse(hdnQuotationId.Value)).UploadedFiles;
            gvDocs.DataBind();

            gvImages.DataSource = new JavaScriptSerializer().Deserialize<PrMasterV2>(ViewState["PrMaster"].ToString()).Bids.Find(b => b.BidId == BidId).SupplierQuotations.Find(q => q.QuotationId == int.Parse(hdnQuotationId.Value)).QuotationImages;
            gvImages.DataBind();

            txtTermsAndConditions.Text = new JavaScriptSerializer().Deserialize<PrMasterV2>(ViewState["PrMaster"].ToString()).Bids.Find(b => b.BidId == BidId).SupplierQuotations.Find(q => q.QuotationId == int.Parse(hdnQuotationId.Value)).TermsAndCondition;
        ...
        protected void btnViewRecommendations_Click(object sender, EventArgs e) {
            ...
            gvRecommenations.DataSource = tabulation.TabulationRecommendations;
        ...
        protected void btnViewApprovals_Click(object sender, EventArgs e) {
            ...
            if (isUploaded > 0) {
                gvApproval.DataSource = new JavaScriptSerializer().Deserialize<PrMasterV2>(ViewState["PrMaster"].ToString()).Bids.Find(b => b.BidId == int.Parse(ViewState["BidId"].ToString())).Tabulations.Find(t => t.TabulationId == TabulationId).TabulationApprovals;
                gvApproval.DataBind();
        ...
        protected void btnTechdocView_Click(object sender, EventArgs e) {
            try {
                ...
                ScriptManager.RegisterClientScriptBlock(Updatepanel1, this.Updatepanel1.GetType(), "none", "<script>    $(document).ready(function () { $('#mdlviewdocsprocCommitee').modal('show');});   </script>", false);
            }
            catch (Exception ex) {
                throw ex;
            }

...
{
    public partial class ViewAllPurchaseRequests : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Session["CompanyId"] != null && Session["UserId"].ToString() != "")
            {
                ...
                ((BiddingAdmin)Page.Master).subTabValue = "ViewAllPurchaseRequests.aspx";
                ((BiddingAdmin)Page.Master).subTabId = "viewAllPrStatusLink";
                ...
                if ((!companyUserAccessController.isAvilableAccess(UserId, CompanyId, 5, 5) && companyLogin.Usertype != "S") && companyLogin.Usertype != "GA")
                {
            ...
            if (!IsPostBack)
            {
                if (UserId != 0)
                {
                    try
                    {
                        PrMasters = pr_MasterController.GetPrListForViewAllPr(CompanyId);
                        BindGV();
                    }
                    catch (Exception ex)
                    {

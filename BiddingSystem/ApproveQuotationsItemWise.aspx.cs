...
namespace BiddingSystem {
    public partial class ApproveQuotationsItemWise : System.Web.UI.Page {
        #region properties
        // static int PrId = 0;
        // static int BidId = 0;
        // static int UserId = 0;
        ...
        protected void Page_Load(object sender, EventArgs e) {
            if (Session["CompanyId"] != null && Session["UserId"].ToString() != "") {
                ...
                if (!companyUserAccessController.isAvilableAccess(int.Parse(Session["UserId"].ToString()), int.Parse(Session["CompanyId"].ToString()), 6, 14) && companyLogin.Usertype != "S") {
            ...
            if (!IsPostBack) {
                if (int.Parse(Session["UserId"].ToString()) != 0) {
                    try {
                        var TabulationIds = Session["ApprovableTabulationIds"] as List<int>;
        ...
        protected void btnHold_Click(object sender, EventArgs e) {
            string remark = "Holded :" + hdnHoldRemarks.Value;
            int result = tabulationApprovalController.HoldRecommendation(int.Parse(hdnTabulationId.Value), int.Parse(hdnRecommendationId.Value), remark, int.Parse(Session["UserId"].ToString()));
            if (result > 0) {
        ...
        private void LoadRejectedBidItems() {
            var PrMasterRej = pr_MasterController.GetPrForQuotationConfirmationRej(int.Parse(ViewState["PrId"].ToString()), int.Parse(Session["CompanyId"].ToString()), int.Parse(Session["UserId"].ToString()), int.Parse(Session["DesignationId"].ToString()));
            ViewState["PrMasterRej"] = new JavaScriptSerializer().Serialize(PrMasterRej);
            Bidding bid = PrMasterRej.Bids.Find(b => b.BidId == int.Parse(ViewState["BidId"].ToString()));
            ...
            var tabulationMaster = PrMasterRej.Bids.Find(b => b.BidId == int.Parse(ViewState["BidId"].ToString())).Tabulations.Where(q => (q.IsSelected == 1) && (q.IsCurrent == 1)).FirstOrDefault();
        ...
        protected void gvRejectedItems_RowDataBound(object sender, GridViewRowEventArgs e) {
            if (e.Row.RowType == DataControlRowType.DataRow) {
                ...
                var PrMaster = new JavaScriptSerializer().Deserialize<PR_Master>(ViewState["PrMasterRej"].ToString());
                ...
                var tabulationMaster = new JavaScriptSerializer().Deserialize<PrMasterV2>(ViewState["PrMasterRej"].ToString()).Bids.Find(b => b.BidId == int.Parse(ViewState["BidId"].ToString())).Tabulations.Where(q => (q.IsSelected == 1) && (q.IsCurrent == 1)).FirstOrDefault();
                List<TabulationDetail> tabulationDetails = new JavaScriptSerializer().Deserialize<PrMasterV2>(ViewState["PrMasterRej"].ToString()).Bids.Find(b => b.BidId == int.Parse(ViewState["BidId"].ToString())).Tabulations.Find(q => q.TabulationId == tabulationMaster.TabulationId).TabulationDetails.Where(x => (x.TabulationId == tabulationMaster.TabulationId)).ToList();
        ...
        protected void gvItems_RowDataBound(object sender, GridViewRowEventArgs e) {
            if (e.Row.RowType == DataControlRowType.DataRow) {
               ...
               // lblNbt.Text = tabulationMaster.NbtAmount.ToString("N2");
                ...
                if (tabulationMaster.ApprovalWasOveridden == 1) {

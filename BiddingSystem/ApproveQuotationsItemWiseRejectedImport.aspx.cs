...
namespace BiddingSystem {
    public partial class ApproveQuotationsItemWiseRejectedImport : System.Web.UI.Page {
        #region properties
        // static int PrId = 0;
        // static int BidId = 0;
        // static int UserId = 0;
        ...
        private void LoadRejectedBidItems() {
            ...
            var RejPrMaster = pr_MasterController.GetPrForQuotationConfirmationRej(int.Parse(ViewState["PrId"].ToString()), int.Parse(Session["CompanyId"].ToString()), int.Parse(Session["UserId"].ToString()), int.Parse(Session["DesignationId"].ToString()));
            ViewState["RejPrMaster"] = new JavaScriptSerializer().Serialize(RejPrMaster);

            Bidding bid = RejPrMaster.Bids.Find(b => b.BidId == int.Parse(ViewState["BidId"].ToString()));
            ...
            var tabulationMaster = RejPrMaster.Bids.Find(b => b.BidId == int.Parse(ViewState["BidId"].ToString())).Tabulations.Where(q => (q.IsSelected == 1) && (q.IsCurrent == 1)).FirstOrDefault();
        ...
        protected void gvRejectedItems_RowDataBound(object sender, GridViewRowEventArgs e) {
            if (e.Row.RowType == DataControlRowType.DataRow) {
                ...
                var RejPrMaster = new JavaScriptSerializer().Deserialize<PR_Master>(ViewState["RejPrMaster"].ToString());
                ...
                var tabulationMaster = RejPrMaster.Bids.Find(b => b.BidId == int.Parse(ViewState["BidId"].ToString())).Tabulations.Where(q => (q.IsSelected == 1) && (q.IsCurrent == 1)).FirstOrDefault();
                List<TabulationDetail> tabulationDetails = RejPrMaster.Bids.Find(b => b.BidId == int.Parse(ViewState["BidId"].ToString())).Tabulations.Find(q => q.TabulationId == tabulationMaster.TabulationId).TabulationDetails.Where(x => (x.TabulationId == tabulationMaster.TabulationId)).ToList();
        ...
        protected void btnPrint1_Click(object sender, EventArgs e) {
          ...
          //  dt.Columns.Add("NbtAmount", typeof(string));
            ...
            Response.AddHeader("content-disposition", "attachment;filename=" + FileName);
            string headerTable = @"<Table><tr><td  colspan='24'><center><h4>Quotation Tabulation For Bid No: B" + bid.BidCode + " </h4></center> </td></tr></Table> <hr>";
            Response.Write(headerTable);
            Response.Charset = "";
            Response.ContentType = "application/vnd.ms-excel";
            using (StringWriter sw = new StringWriter()) {
                ...
                for (int i = 0; i < gv.Rows.Count; i++) {
                    GridViewRow currentRow = gv.Rows[i];
                    if (currentRow.Cells[22].Text == "Selected") {

...
{
    public partial class ConfirmQuotations : System.Web.UI.Page
    {
        #region properties
        static int PrId = 0;
        static int BidId = 0;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Session["CompanyId"] != null && Session["UserId"].ToString() != "")
            {
                ...
                if (!companyUserAccessController.isAvilableAccess(UserId, CompanyId, 6, 14) && companyLogin.Usertype != "S")
                {
            ...
            if (!IsPostBack)
            {
                if (UserId != 0)
                {
                    try
                    {
                        ...
                        PrMaster = pr_MasterController.GetPrForQuotationConfirmation(PrId, CompanyId);
        ...
        protected void btnReject_Click(object sender, EventArgs e)
        {
            ...
            int result = biddingController.ConfirmOrRejectSelectedQuotation(int.Parse(hdnBidId.Value), 2, hdnRejectRemarks.Value, UserId);

            if (result > 0)
            {
                ScriptManager.RegisterClientScriptBlock(Updatepanel1, this.Updatepanel1.GetType(), "none", "<script>    $(document).ready(function () { swal({ type: 'success',title: 'SUCCESS',text:'Your work has been saved', showConfirmButton: false,timer: 1500}).then((result) => { window.location = 'ViewPrForQuotationConfirmation.aspx' });; });   </script>", false);
        ...
        protected void btnApprove_Click(object sender, EventArgs e)
        {
            ...
            if ((decimal.Parse(hdnSubTotal.Value) != quotation.SubTotal) && viewedQuotation == 1)
            {
                ...
                if (result > 0)
                {
                    result = biddingController.ConfirmOrRejectSelectedQuotation(int.Parse(hdnBidId.Value), 1, hdnApprovalRemarks.Value, UserId);
            ...
            {
                int result = biddingController.ConfirmOrRejectSelectedQuotation(int.Parse(hdnBidId.Value), 1, hdnApprovalRemarks.Value, UserId);

...
namespace BiddingSystem {
    public partial class GenerateGRNNew : System.Web.UI.Page {
        ...
        PR_DetailController pRDetailsV2 = ControllerFactory.CreatePR_DetailController();
        ...
        private void Populate() {
            ...
            ViewState["PaymentMethod"] = poMaster.PaymentMethod;
            ViewState["QuotationId"] = poMaster.QuotationId;
            ...
            try {
                ...
                for (int i = 0; i < gvPoItems.Rows.Count; i++) {
                    ...
                    if ((gvPoItems.Rows[i].Cells[6].FindControl("txtQuantity") as TextBox).Text != "" && decimal.Parse((gvPoItems.Rows[i].Cells[7].FindControl("txtQuantity") as TextBox).Text) > 0) {
                        GrnDetails grnDetail = new GrnDetails {
                            ...
                            ItemPrice = decimal.Parse(gvPoItems.Rows[i].Cells[11].Text),
                ...
                if (grnMaster.GrnDetailsList.Count > 0) {
                    if (hdnHasMore.Value == "1") {
                        ...
                        int ParentPrId = pR_MasterController.GetParentPrforCoverinPr(PoId);
                        int UserId = int.Parse(Session["UserId"].ToString());
                        List<TempCoverigPr> PrDetails = new List<TempCoverigPr>();

                        for (int k = 0; k < gvPoItems.Rows.Count; k++) {

                            decimal Quantity = decimal.Parse((gvPoItems.Rows[k].Cells[8].FindControl("txtQuantity") as TextBox).Text);
                            decimal PendingQty = decimal.Parse(gvPoItems.Rows[k].Cells[7].Text);
                            int PodId = int.Parse(gvPoItems.Rows[k].Cells[1].Text);
                            decimal ExtraQty = (Quantity - PendingQty);


                            if (ExtraQty > 0) {
                                TempCoverigPr details = new TempCoverigPr();
                                details.ItemId = int.Parse(gvPoItems.Rows[k].Cells[2].Text);
                                details.ExtraQty = ExtraQty;
                                details.PodId = PodId;
                                details.ReceivedQty = Quantity;
                                details.PrdId = pRDetailsV2.FetchPR_DetailsByPrIdAndItemId(ParentPrId, details.ItemId).PrdId;

                                PrDetails.Add(details);
                            }
                        }

                        Temp = ControllerFactory.CreateTempCoveringPrController().AddCoveringPrData(PrDetails, ParentPrId, PoId);
                        if (Temp > 0) {
                            int CoveringPrBid = pR_MasterController.CreateCoveringPr(PoId, ParentPrId, UserId, int.Parse(ViewState["SupplierId"].ToString()), int.Parse(ViewState["QuotationId"].ToString()));
                            if (CoveringPrBid > 0) {
                                List<SupplierQuotationItem> supplierQuotationItemList = supplierQuotationItemController.GetAllQuotationItems(int.Parse(ViewState["QuotationId"].ToString()), int.Parse(Session["CompanyId"].ToString()));
                                List<TempQuotationsForCoverinPr> QuotatioDetails = new List<TempQuotationsForCoverinPr>();
                                decimal TotalVat = 0;
                                decimal TotalSubTotal = 0;
                                decimal TotalNetTotal = 0;

                                for (int j = 0; j < supplierQuotationItemList.Count; j++) {
                                    decimal unitPrice = supplierQuotationItemList[j].UnitPrice;
                                    int ItemId = supplierQuotationItemList[j].ItemId;
                                    int QuotationId = supplierQuotationItemList[j].QuotationId;
                                    int QuotationItemId = supplierQuotationItemList[j].QuotationItemId;
                                    int SupplierId = supplierQuotationItemList[j].SupplierId;
                                    
                                    for (int k = 0; k < gvPoItems.Rows.Count; k++) {
                                        if (int.Parse(gvPoItems.Rows[k].Cells[2].Text) == ItemId) {
                                            decimal Quantity = decimal.Parse((gvPoItems.Rows[k].Cells[8].FindControl("txtQuantity") as TextBox).Text);
                                            decimal PendingQty = decimal.Parse(gvPoItems.Rows[k].Cells[7].Text);
                                            decimal ExtraQty = (Quantity - PendingQty);
                                            if (ExtraQty > 0) {
                                                TempQuotationsForCoverinPr Q_details = new TempQuotationsForCoverinPr();
                                                Q_details.SubTotal = (ExtraQty * unitPrice);
                                                if (int.Parse(gvPoItems.Rows[k].Cells[16].Text) == 1) {
                                                    Q_details.Vat = (Q_details.SubTotal * 0.08m);
                                                }
                                                else {
                                                    Q_details.Vat = 0;
                                                }
                                                //Q_details.Vat = (Q_details.SubTotal * 0.08m);
                                                Q_details.NetTotal = (Q_details.SubTotal + Q_details.Vat);
                                                Q_details.Qty = ExtraQty;
                                                Q_details.ItemId = int.Parse(gvPoItems.Rows[k].Cells[2].Text);
                                                Q_details.QuotationId = QuotationId;
                                                Q_details.QuotationItemId = QuotationItemId;
                                                Q_details.UnitPrice = unitPrice;
                                                Q_details.SupplierId = SupplierId;

                                                TotalVat = TotalVat + Q_details.Vat;
                                                TotalSubTotal = TotalSubTotal + Q_details.SubTotal;
                                                TotalNetTotal = TotalNetTotal + Q_details.NetTotal;

                                                QuotatioDetails.Add(Q_details);
                                            }
                                        }

                                    }
                                }
                                int Q_Temp = ControllerFactory.CreateTempQuotatioForCoveringPrController().AddQuotationData(QuotatioDetails, ParentPrId);
                                if (Q_Temp > 0) {
                                    int CoveringQuotations = supplierQuotationItemController.CreateCoveringPrQuotations(ParentPrId, TotalVat, TotalSubTotal, TotalNetTotal, int.Parse(Session["UserId"].ToString()));

                                }
                            }
                            int deletePrTemp = ControllerFactory.CreateTempCoveringPrController().DeleteCoveringPrData(ParentPrId);
                            int deleteQuotationTemp = ControllerFactory.CreateTempQuotatioForCoveringPrController().DeleteQuotationData(ParentPrId);
                   ...
                   // int result = 0;
                    if (result > 0) {
                        ...
                        if (Temp > 0) {
                            ...
                            int UpdateGrnCoverigPR = ControllerFactory.CreateGrnController().UpdateGrnCoverigPR(result, int.Parse(Session["CompanyId"].ToString()));

                            if (UpdateGrnCoverigPR > 0) {

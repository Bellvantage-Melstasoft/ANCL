...
{
    public partial class CustomerApprovePONew : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Session["CompanyId"] != null && Session["UserId"].ToString() != null)
            {
                ...
                ((BiddingAdmin)Page.Master).subTabValue = "CustomerApprovePONew.aspx";
                ((BiddingAdmin)Page.Master).subTabId = "CustomerApprovePONewLink";
              ...
              //  UserId = Session["UserId"].ToString();
               var companyLogin = companyLoginController.GetUserbyuserId(int.Parse(Session["UserId"].ToString()));
                ...
                if ((!companyUserAccessController.isAvilableAccess(int.Parse(Session["UserId"].ToString()), int.Parse(Session["CompanyId"].ToString()), 6, 22) && companyLogin.Usertype != "S") && companyLogin.Usertype != "GA")
                {
            ...
            if (!IsPostBack)
            {
                if (int.Parse(Session["UserId"].ToString()) != 0)
                {
                    try
                    {
                        ...
                        pOMasterListByDepartmentid = pOMasterController.GetPosForApproval(int.Parse(Session["CompanyId"].ToString()), int.Parse(Session["UserId"].ToString()));

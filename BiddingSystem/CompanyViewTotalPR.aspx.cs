...
{
    public partial class CompanyViewTotalPR : System.Web.UI.Page
    {
        ...
        List<PR_Master> pr_Master2 = new List<PR_Master>();
        List<PR_Details> pr_Details2 = new List<PR_Details>();
        List<PR_Master> pr_Master3 = new List<PR_Master>();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Session["CompanyId"] != null && Session["UserId"].ToString() != "")
            {
                ...
                ((BiddingAdmin)Page.Master).subTabId = "rejectPRLink";
            ...
            if (!IsPostBack)
            {
                try
                {
                    ...
                    gvPRMastre.DataBind();
                    pr_Master2 = pr_MasterController.FetchRejectedPr(int.Parse(CompanyId));
                    gvPRMastre2.DataSource = pr_Master2;
                    gvPRMastre2.DataBind();
                    pr_Master3 = pr_MasterController.FetchApprovePRDataByDeptId(int.Parse(CompanyId)).OrderByDescending(x => x.PrId).ToList();
                    gvPurchaseRequest.DataSource = pr_Master3;
        ...
        protected void OnRowDataBound1(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    {
                        pr_Details = pr_DetailController.FetchDetailsApprovedPR(item.PrId, item.DepartmentId).ToList();
                        pr_DetailsDup.AddRange(pr_Details);
                    }
                    string prId = gvPRMastre.DataKeys[e.Row.RowIndex].Value.ToString();
                    GridView gvPRApprovedDetails = e.Row.FindControl("gvPRApprovedDetails") as GridView;

                    gvPRApprovedDetails.DataSource = pr_DetailsDup.Where(x => x.PrId == int.Parse(prId));
                    gvPRApprovedDetails.DataBind();
                }
            }
            catch (Exception ex)
            {
        ...
        protected void OnRowDataBound2(object sender, GridViewRowEventArgs e)
        {
            try
            {
                ...
                {
                    List<PR_Details> pr_DetailsDup = new List<PR_Details>();
                    foreach (var item in pr_Master2)
                    {
                        pr_Details2 = pr_DetailController.FetchDetailsRejectedPR(item.PrId, item.DepartmentId).ToList();
                        pr_DetailsDup.AddRange(pr_Details2);
                    }
                    string prId = gvPRMastre2.DataKeys[e.Row.RowIndex].Value.ToString();
                    GridView gvRejectedPRDetails = e.Row.FindControl("gvRejectedPRDetails") as GridView;

                    gvRejectedPRDetails.DataSource = pr_DetailsDup.Where(x => x.PrId == int.Parse(prId));
                    gvRejectedPRDetails.DataBind();
                }
            }
            catch (Exception ex)
            {

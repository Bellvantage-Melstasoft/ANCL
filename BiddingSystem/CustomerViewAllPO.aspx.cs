...
{
    public partial class CustomerViewAllPO : System.Web.UI.Page
    {
       ...
       // int DesignationId = 0;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Session["CompanyId"] != null && Session["UserId"].ToString() != null)
            {
                ...
                ((BiddingAdmin)Page.Master).subTabValue = "CustomerViewAllPO.aspx";
                ((BiddingAdmin)Page.Master).subTabId = "ViewAllPOLink";
              ...
              //  UserId = Session["UserId"].ToString();
               var companyLogin = companyLoginController.GetUserbyuserId(int.Parse(Session["UserId"].ToString()));
                ...
                if ((!companyUserAccessController.isAvilableAccess(int.Parse(Session["UserId"].ToString()), int.Parse(Session["CompanyId"].ToString()), 6, 20) && companyLogin.Usertype != "S") && companyLogin.Usertype != "GA")
                {
                ...
                if (int.Parse(Session["UserId"].ToString()) != 0)
                {
                    try
                    {
                      ...
                      //// {
                            ...
                            gvPurchaseOrder.DataSource = pOMasterController.ViewAllPOS(int.Parse(Session["CompanyId"].ToString()), LocalTime.Now, prCode, poCode, CategoryIds, warehouseIds, poType);
       ...
       /* private void LoadCompanyWarehouses() {
        ...
        {
            ddlSupplier.DataSource = ControllerFactory.CreateSupplierController().GetSupplierList();
            ddlSupplier.DataValueField = "SupplierId";
            ddlSupplier.DataTextField = "SupplierName";
            ddlSupplier.DataBind();
            ddlSupplier.Items.Insert(0, new ListItem("Not Found", "0"));
        }
        ...
        protected void btnSearch_Click(object sender, EventArgs e) {
            ...
            try {
                if (chkMonth.Checked || chkPrCode.Checked || chkCategory.Checked || chkPoCode.Checked || chkSupplier.Checked ) {
                   ...
                   /* List<int> warehouseIds = new List<int>();
                    ...
                    if (chkSupplier.Checked)
                    {
                        ...
                        {
                            if (ddlSupplier.Items[i].Selected)
                            {
                                supplierIds.Add(int.Parse(ddlSupplier.Items[i].Value));
                            }
                        }
                    }

                    List<int> warehouseIds = new List<int>();
                    gvPurchaseOrder.DataSource = pOMasterController.ViewAllPOS(int.Parse(Session["CompanyId"].ToString()), date, prCode, poCode, CategoryIds, warehouseIds, poType, supplierIds);
                    gvPurchaseOrder.DataBind();

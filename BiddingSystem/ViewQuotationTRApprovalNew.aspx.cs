...
{
    public partial class ViewQuotationTRApprovalNew : System.Web.UI.Page
    {
        ...
        public static string ApproveClick(SupplierQuotationItem SupQutItem)
        {
            try
            {
                if (HttpContext.Current.Session["UserId"] != null)
                {
                    ...
                    var PrMasterDetails = ControllerFactory.CreatePR_MasterController().getPRMasterDetailByPrId(Convert.ToInt32(HttpContext.Current.Session["PrID"].ToString()));
                    int CategoryId = PrMasterDetails.PrCategoryId;
                    int PurchaseType = PrMasterDetails.PurchaseType;
                    ...
                    try {
                        if (PrMasterDetails.PurchaseProcedure != 2) { 
                        //int bidId = int.Parse(HttpContext.Current.Session["BidId"].ToString());
                        ...
                        if (approve > 0) {
                            ...
                            List<BiddingItem> Biddinglist = ControllerFactory.CreateBiddingController().GetPrdIdByBidId(bidId);
                            SupplierQuotation supplierQuotation = ControllerFactory.CreateSupplierQuotationController().GetSupplierQuotationsForCoveringPR(bidId);
                            List<SupplierQuotationItem> supplierQuotationItem = ControllerFactory.CreateSupplierQuotationItemController().GetAllQuotationItemsForCoveringPR(supplierQuotation.QuotationId);


                            if (approve > 0) {
                                for (int i = 0; i < Biddinglist.Count; i++) {
                                    int prdId = Biddinglist[i].PrdId;
                                    int UpdateTabulation = ControllerFactory.CreateTabulationMasterController().UpdateTabulationForCoveringPR(supplierQuotation, supplierQuotationItem, int.Parse(HttpContext.Current.Session["UserId"].ToString()), SupQutItem.Remark, bidId, CategoryId, PurchaseType,  int.Parse(HttpContext.Current.Session["DesignationId"].ToString()));
                                    if (UpdateTabulation > 0) { 
                                    ControllerFactory.CreatePR_DetailController().UpdatePRStatuss(int.Parse(HttpContext.Current.Session["UserId"].ToString()), prdId, "BIDCMPRSN", "TABREVAPPROVD");
                                    }
                                }
                            }
        ...
        public void BindBasicDetails(int prId,int CompanyId)
        {
            ...
            if (PrMaster.PurchaseProcedure == 2) {
                var ParentPR = pr_MasterController.GetPrForQuotationComparison(PrMaster.ClonedFromPR, CompanyId);
        ...
        private void LoadGV()
        {
            try
            {
                ...
                if (int.Parse(ViewState["ClnedPr"].ToString()) == 1) {
                    PrMaster.Bids.ForEach(b => { b.IsClnedPr = 1; });
                }

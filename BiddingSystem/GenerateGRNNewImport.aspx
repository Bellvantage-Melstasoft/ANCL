<%@ Page Title="" Language="C#" MasterPageFile="~/BiddingAdmin.Master" AutoEventWireup="true" CodeBehind="GenerateGRNNewImport.aspx.cs" EnableViewState="true" Inherits="BiddingSystem.GenerateGRNNewImport" %>
                                        ...
                                        <asp:GridView runat="server" ID="gvPoItems" AutoGenerateColumns="false"
                                            ...
                                            <Columns>
                                                ...
                                                <asp:TemplateField HeaderText="VAT">
                                                    <ItemTemplate>
                                                        <asp:TextBox ID="txtVat" Text='<%# Eval("VatAmountDisplay") %>'
                                                            type="number" step=".01" min="0" runat="server" Enabled="false"
                                                ...
                                                <asp:BoundField DataField="PoPurchaseType" HeaderText="Po Purchase Type" HeaderStyle-CssClass="hidden" ItemStyle-CssClass="hidden" />
                                                <asp:BoundField DataField="VatAmount" HeaderText="Vat Amount" HeaderStyle-CssClass="hidden" ItemStyle-CssClass="hidden"/>
                                                 <asp:BoundField DataField="ItemPrice" HeaderText="Price" HeaderStyle-CssClass="hidden" ItemStyle-CssClass="hidden"/>
                                                <%-- <asp:TemplateField HeaderText="Free QTY">
                                                    <ItemTemplate>
        ...
        Sys.Application.add_load(function () {
            $(function () {
                ...
                function calculate(elmt) {
                    ...
                    var itemPrice = parseFloat($(row).eq(22).html().replace(',', ''))
                    ...
                    if ($(row).eq(16).html() == "1") {
                        ...
                        if ($(row).eq(20).html() == "2") {
                            if (qty > 0) {
                                vat = parseFloat($(row).eq(21).html().replace(',', ''))
                            }
                            else {
                                vat = 0;
                            }
                    ...
                    click: function () {
                        ...
                        if (hasMore) {
                            swal.fire({
                                ...
                                html: "You have exceeded the requested quantity of one or more items. New POs as Covering PO will be generated for those items. Are you sure you want to continue?",
                                type: 'warning',

...
{
    public partial class ViewAllMRNRequestsEdit : System.Web.UI.Page
    {
        ...
        {
            Warehouseid = int.Parse(Session["WarehouseID"].ToString());
            MrnMaster = MrnMasterController.FetchMRNToEdit(CompanyId, subDepartmentId).OrderByDescending(x => x.CreatedDate).ToList();
            MrnMaster = MrnMaster.FindAll(t => t.CreatedBy == UserId).ToList();
            ...
            List<MrnMaster> tempmrn = MrnMaster.Where(x => x.MrnDetails.Count == 0).ToList(); // Getting MRN which have not added any Item (When Development Testing)
            MrnMaster = MrnMaster.Where(x => x.MrnDetails.Count > 0).ToList();
            MrnMaster.AddRange(tempmrn); // Added Them if needed
        ...
        protected void btnEdit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Response.Redirect("EditMRN_V1.aspx?MrnId=" + mrnId);
            }
            catch (Exception ex)
            {
                throw ex;

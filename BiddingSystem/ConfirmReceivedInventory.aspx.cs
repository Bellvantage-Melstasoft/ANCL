...
{
    public partial class ConfirmReceivedInventory : System.Web.UI.Page
    {
       ...
       // int CompanyId = 0;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Session["CompanyId"] != null && Session["UserId"].ToString() != null)
            {
                ...
                ((BiddingAdmin)Page.Master).subTabValue = "ConfirmReceivedInventory.aspx";
                ((BiddingAdmin)Page.Master).subTabId = "ConfirmReceivedInventoryLink";
               ...
               ViewState["CompanyId"] = int.Parse(Session["CompanyId"].ToString());
                ...
                if ((!companyUserAccessController.isAvilableAccess(int.Parse(Session["UserId"].ToString()), int.Parse(Session["CompanyId"].ToString()), 12, 9) && companyLogin.Usertype != "S") &&  companyLogin.Usertype != "GA")
                {
            ...
            if (!IsPostBack)
            {
                if ((Session["UserDepartments"] != null && (Session["UserDepartments"] as List<UserSubDepartment>).Where(d => d.IsHead == 1).Count() > 0)) {
                    ...
                    try {
                        gvDeliveredInventory.DataSource = mrndinController.fetchDeliveredMrndINListToConfirm((Session["UserDepartments"] as List<UserSubDepartment>).Where(d => d.IsHead == 1).Select(d => d.DepartmentId).ToList());
                        gvDeliveredInventory.DataBind();

                        gvReceivedInventory.DataSource = mrndinController.fetchConfirmedMrndINList((Session["UserDepartments"] as List<UserSubDepartment>).Where(d => d.IsHead == 1).Select(d => d.DepartmentId).ToList());
                    ...
                    gvDeliveredInventory.DataSource = mrndinController.fetchDeliveredMrndINListToConfirmByCompanyId(int.Parse(Session["CompanyId"].ToString()));
                    gvDeliveredInventory.DataBind();
        ...
        protected void btnReceive_Click(object sender, EventArgs e)
        {
           // GridViewRow row = (GridViewRow)(((Button)sender).NamingContainer);
            ...
            int result = mrndinController.updateIssueNoteAfterReceived(mrndin);

            if(result>0)
            {
                result = mrnController.updateMRNDReceivedQty(int.Parse(hdnMrndID.Value), decimal.Parse(hdnIssuesQty.Value));
                ...
                if(result>0)
                {
                    ...
                    mrnDetailsStatusLogController.InsertLogConfirmation(int.Parse(hdnMrndID.Value),int.Parse(Session["UserId"].ToString()));
                    gvDeliveredInventory.DataSource = mrndinController.fetchDeliveredMrndINListToConfirm((Session["UserDepartments"] as List<UserSubDepartment>).Where(d => d.IsHead == 1).Select(d => d.DepartmentId).ToList());
                    gvDeliveredInventory.DataBind();

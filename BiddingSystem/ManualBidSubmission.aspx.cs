...
{
    public partial class ManualBidSubmission : System.Web.UI.Page
    {
        ...
        protected void ddlSuppliers_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (ddlSuppliers.SelectedIndex != 0)
            {
                ...
                var listSupplierAgent = supplierController.getSupplierAgent();
        ...
        private DataTable CreateDataTable()
        {
            ...
            dt.Columns.Add("HsId");
            dt.Columns.Add("Qty");
            // dt.Columns.Add("ItemReferenceCode");
        ...
        protected void btnEdit_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < quotation.QuotationItems.Count; i++)
            {
                ...
                newRow["Brand"] = quotation.QuotationItems[i].Brand.ToString();// Later chnage
            ...
            try
            {
                ...
                if (btnSubmit.Text == "Submit Quotation")
                {
                    ...
                    for (int i = 0; i < gvBidItems.Rows.Count; i++)
                    {
                        ...
                        if ((description != "" || int.Parse(ViewState["PurchaseType"].ToString()) == 2) && unitPrice != 0) {
                            ...
                            quotationItem.EstimatedPrice = decimal.Parse(gvBidItems.Rows[i].Cells[10].Text);
                    ...
                    foreach (var item in quotation.QuotationItems)
                    {
                        for (int i = 0; i < gvBidItems.Rows.Count; i++)
                        {
                            if (Convert.ToInt32(gvBidItems.Rows[i].Cells[0].Text) == item.BiddingItemId)
                            {
                                ...
                                item.UnitPrice = decimal.Parse((gvBidItems.Rows[i].FindControl("txtUnitPrice") as TextBox).Text);
                                ...
                                item.Validity = (gvBidItems.Rows[i].FindControl("txtvalidity") as TextBox).Text != "" ? DateTime.ParseExact((gvBidItems.Rows[i].FindControl("txtvalidity") as TextBox).Text, "yyyy-MM-dd", CultureInfo.InvariantCulture) : DateTime.Now;
        ...
        protected void ddlTerms_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (selectedValues != "0" && selectedValues != "LOCAL")
            {
        ...
        protected void btnQuoItemDelete_Click(object sender, EventArgs e)
        {
            ...
            try
            {
              ...
              //supplierQuotation.DeleteSubmittedSupplierQuotation(quotationId, itemId, quotationItemId);
                LoadGV();
            }
            catch (Exception ex)
            {
        ...
        protected void btnViewQuotationSupportiveDocuments_Click(object sender, EventArgs e)
        {
            try
            {
                gvQuotationDocs.DataSource = new JavaScriptSerializer().Deserialize<Bidding>(ViewState["Bid"].ToString()).SupplierQuotations.Find(b => b.QuotationId == int.Parse(ViewState["QuotationId"].ToString())).UploadedFiles;

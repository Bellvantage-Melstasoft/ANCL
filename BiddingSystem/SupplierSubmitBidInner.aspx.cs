...
{
    public partial class SupplierSubmitBidInner : System.Web.UI.Page
    {
                ...
                try
                {
                    ...
                    if (Status == "A")
                    {
                        ...
                        foreach (var item in _supplierQuotationAlreadyBid)
                        {
                            ...
                            txtTotalPrice.Text = ((int)decimal.Parse(itemQuontity.ToString()) * decimal.Parse(item.PerItemPrice.ToString())).ToString();

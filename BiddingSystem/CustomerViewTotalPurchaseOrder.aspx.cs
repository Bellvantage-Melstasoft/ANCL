...
{
    public partial class CustomerViewTotalPurchaseOrder : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Session["CompanyId"] != null && Session["UserId"].ToString() != null)
            {
                ...
                if (!companyUserAccessController.isAvilableAccess(int.Parse(UserId), CompanyId, 6, 9) && companyLogin.Usertype != "S")
                {
            ...
            if (!IsPostBack)
            {
                if (int.Parse(UserId) != 0)
                {
                    try
                    {
                        ...
                        _grnDetails = grnDetailsController.GetGrnDetailsAll(CompanyId).Where(x => (x.IsGrnRaised != 1 && x.IsGrnApproved == 2) || x.IsGrnRaised == 1).ToList();

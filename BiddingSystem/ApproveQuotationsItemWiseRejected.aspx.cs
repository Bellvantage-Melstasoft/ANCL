...
namespace BiddingSystem {
    public partial class ApproveQuotationsItemWiseRejected : System.Web.UI.Page {
        #region properties
        // static int PrId = 0;
        // static int BidId = 0;
        // static int UserId = 0;
        ...
        QuotationApprovalController quotationApprovalController = ControllerFactory.CreateQuotationApprovalController();
        ...
        protected void Page_Load(object sender, EventArgs e) {
            if (Session["CompanyId"] != null && Session["UserId"].ToString() != "") {
                ...
                if (!companyUserAccessController.isAvilableAccess(int.Parse(Session["UserId"].ToString()), int.Parse(Session["CompanyId"].ToString()), 13, 4) && companyLogin.Usertype != "S") {
            ...
            if (!IsPostBack) {
                if (int.Parse(Session["UserId"].ToString()) != 0) {
                    try {
                        ...
                        var TabulationIds = Session["RejectedApprovableTabulationIds"] as List<int>;
                        ViewState["TabulationIds"] = new JavaScriptSerializer().Serialize(TabulationIds);

                        ViewState["PrId"] = int.Parse(Request.QueryString.Get("PrId"));
                        var PrMaster = pr_MasterController.GetPrForQuotationConfirmation(int.Parse(ViewState["PrId"].ToString()), int.Parse(Session["CompanyId"].ToString()), TabulationIds, int.Parse(Session["UserId"].ToString()), int.Parse(Session["DesignationId"].ToString()));
                        ...
                        //    btnOverrideReject.Visible = true;
                        //}
                        //else {
                        //    btnOverrideRecomnd.Visible = false;
                        //    btnOverrideReject.Visible = false;
                        //}
        ...
        protected void OnRowDataBound_gvRejectedTabulations(object sender, GridViewRowEventArgs e) {
            //foreach (GridViewRow row in gvRejectedTabulations.Rows) {
            //    if (row.RowType == DataControlRowType.DataRow) {
            //        for (int i = 0; i < gvRejectedTabulations.Rows.Count; i++) {
            //            Button btn = row.FindControl("btnClone1") as Button;
            //            if (i == gvRejectedTabulations.Rows.Count - 1) {
            //                btn.Enabled = false;
            //            }
            //        }
            //    }
        ...
        protected void btnRejectedViewNew_Click(object sender, EventArgs e) {
            ...
            ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "none", "<script>    $(document).ready(function () { $('#mdlRejectedQuotationNew').modal('show'); });   </script>", false);

        }

        private void LoadRejectedBidItems() {
            //var PrMaster = new JavaScriptSerializer().Deserialize<PR_Master>(ViewState["PrMaster"].ToString());
            List<int> TabulationIds = new List<int> ();
            for (int i = 0; i < gvRejectedTabulations.Rows.Count; i++) {
                int TabulationId = int.Parse(gvRejectedTabulations.Rows[i].Cells[0].Text);
                TabulationIds.Add(TabulationId);
            }
            var RejectedPrMaster = pr_MasterController.GetPrForQuotationRejected(int.Parse(ViewState["PrId"].ToString()), int.Parse(Session["CompanyId"].ToString()), TabulationIds, int.Parse(Session["UserId"].ToString()), int.Parse(Session["DesignationId"].ToString()));
            ViewState["RejectedPrMaster"] = new JavaScriptSerializer().Serialize(RejectedPrMaster);
            Bidding bid = RejectedPrMaster.Bids.Find(b => b.BidId == int.Parse(ViewState["BidId"].ToString()));
            ...
            var tabulationMaster = RejectedPrMaster.Bids.Find(b => b.BidId == int.Parse(ViewState["BidId"].ToString())).Tabulations.Where(q => (q.IsSelected == 1) && (q.IsCurrent == 1)).FirstOrDefault();
            ...
            gvRejectedItems.DataSource = items;
            gvRejectedItems.DataBind();
        ...
        protected void gvRejectedItems_RowDataBound(object sender, GridViewRowEventArgs e) {
            if (e.Row.RowType == DataControlRowType.DataRow) {
                ...
                int BidItemId = int.Parse(gvRejectedItems.DataKeys[e.Row.RowIndex].Value.ToString());
                ...
                var RejectedPrMaster = new JavaScriptSerializer().Deserialize<PR_Master>(ViewState["RejectedPrMaster"].ToString());
                ...
                var tabulationMaster = RejectedPrMaster.Bids.Find(b => b.BidId == int.Parse(ViewState["BidId"].ToString())).Tabulations.Where(q => (q.IsSelected == 1) && (q.IsCurrent == 1)).FirstOrDefault();
                List<TabulationDetail> tabulationDetails = RejectedPrMaster.Bids.Find(b => b.BidId == int.Parse(ViewState["BidId"].ToString())).Tabulations.Find(q => q.TabulationId == tabulationMaster.TabulationId).TabulationDetails.Where(x => (x.TabulationId == tabulationMaster.TabulationId)).ToList();
                ...
                for (int i = 0; i < quotationItems.Count; i++) {
                    ...
                    newRow["Description"] = tabulationDetail.Description;
        ...
        private void LoadBidItems() {
            ...
            ViewState["tabulationMaster"] = new JavaScriptSerializer().Serialize(tabulationMaster);
            int Override = tabulationMaster.CanLoggedInUserOverrideApproval;
            hdnTabulationId.Value = tabulationMaster.TabulationId.ToString();
            if (Override == 1) {
                btnOverrideRecomnd.Visible = true;
        ...
        protected void gvItems_RowDataBound(object sender, GridViewRowEventArgs e) {
            if (e.Row.RowType == DataControlRowType.DataRow) {
                ...
                lblTotal.Text = tabulationMaster.SubTotal.ToString("N2");
                lblVat.Text = tabulationMaster.VatAmount.ToString("N2");
               // lblNbt.Text = tabulationMaster.NbtAmount.ToString("N2");
                lblNetTotal.Text = tabulationMaster.NetTotal.ToString("N2");
                lblRemark.Text = tabulationMaster.SelectionRemarks.ToString();

                if (tabulationMaster.RecommendationWasOveridden == 1) {
                    lblTabOverridden.Visible = true;
                    lblTabOverridden.Text = "Tabulation Approval was Overridden by " + tabulationMaster.ApprovalOverriddenByName + " on "+ tabulationMaster.ApprovalOverriddenOn.ToString("dd MMMM yyyy")+ " ";
                }
            }
        ...
        protected void btnViewAttachments_Click(object sender, EventArgs e) {
            ...
            var BidId = int.Parse(ViewState["BidId"].ToString());
            int quotationId = int.Parse(((sender as Button).NamingContainer as GridViewRow).Cells[2].Text);
            gvDocs.DataSource = new JavaScriptSerializer().Deserialize<PrMasterV2>(ViewState["PrMaster"].ToString()).Bids.Find(b => b.BidId == BidId).SupplierQuotations.Find(q => q.QuotationId == quotationId).UploadedFiles;
            gvDocs.DataBind();

            gvImages.DataSource = new JavaScriptSerializer().Deserialize<PrMasterV2>(ViewState["PrMaster"].ToString()).Bids.Find(b => b.BidId == BidId).SupplierQuotations.Find(q => q.QuotationId == quotationId).QuotationImages;
            gvImages.DataBind();

            txtTermsAndConditions.Text = new JavaScriptSerializer().Deserialize<PrMasterV2>(ViewState["PrMaster"].ToString()).Bids.Find(b => b.BidId == BidId).SupplierQuotations.Find(q => q.QuotationId == quotationId).TermsAndCondition;
        ...
        protected void btnOverrideReject_Click(object sender, EventArgs e) {
            ...
            int result = tabulationMasterController.OverrideApproval(int.Parse(hdnTabulationId.Value), int.Parse(Session["UserId"].ToString()), int.Parse(Session["DesignationId"].ToString()), hdnRejectRemarks.Value, 2);

            if (result > 0) {
                ScriptManager.RegisterClientScriptBlock(Updatepanel1, this.Updatepanel1.GetType(), "none", "<script>    $(document).ready(function () { swal({ type: 'success',title: 'SUCCESS',text:'Your work has been saved', showConfirmButton: false,timer: 1500}).then((result) => { window.location = 'ViewPrForQuotationApprovalNew.aspx' });; });   </script>", false);
            }
            else {
        ...
        protected void btnOverrideApprove_Click(object sender, EventArgs e) {
            ...
            int result = tabulationMasterController.OverrideApproval(int.Parse(hdnTabulationId.Value), int.Parse(Session["UserId"].ToString()), int.Parse(Session["DesignationId"].ToString()), hdnRejectRemarks.Value, 1);
        ...
        protected void btnOverrideApprove1_Click(object sender, EventArgs e) {
            ...
            int result = tabulationApprovalController.TabulationApprovalOverride(int.Parse(hdnTabulationId.Value), int.Parse(Session["UserId"].ToString()), int.Parse(Session["DesignationId"].ToString()), hdnApprovalRemarks.Value, 1);
            ...
            if (Tabulation.IsApproved == 1) {
                for (int i = 0; i < Biddinglist.Count; i++) {
                    int prdId = Biddinglist[i].PrdId;
                    ControllerFactory.CreatePR_DetailController().UpdatePRStatuss(int.Parse(HttpContext.Current.Session["UserId"].ToString()), prdId, "CRTPO", "OVRRD_APP");

                }
            }

            if (Tabulation.IsApproved == 2) {
                for (int i = 0; i < Biddinglist.Count; i++) {
                    int prdId = Biddinglist[i].PrdId;
                    ControllerFactory.CreatePR_DetailController().UpdatePRStatuss(int.Parse(HttpContext.Current.Session["UserId"].ToString()), prdId, "QTATION_APRVL_RJCTED", "OVRRD_REJ");
        ...
        protected void btnOverrideReject1_Click(object sender, EventArgs e) {
            ...
            int result = tabulationApprovalController.TabulationApprovalOverride(int.Parse(hdnTabulationId.Value), int.Parse(Session["UserId"].ToString()), int.Parse(Session["DesignationId"].ToString()), hdnRejectRemarks.Value, 2);
        ...
        protected void btnViewRecommendations_Click(object sender, EventArgs e) {
            ...
            TabulationMaster tabulation = new JavaScriptSerializer().Deserialize<PrMasterV2>(ViewState["PrMaster"].ToString()).Bids.Find(b => b.BidId == int.Parse(ViewState["BidId"].ToString())).Tabulations.Find(t => t.TabulationId == int.Parse(hdnTabulationId.Value));

            lblWasRecommendationOverriden.Text = tabulation.RecommendationWasOveridden == 1 ? "Yes" : "No";
            lblRecommendationOveriddenByName.Text = tabulation.RecommendationOveriddenByName == null ? "-" : tabulation.RecommendationOveriddenByName;
            lblRecommendationOveriddenOn.Text = tabulation.RecommendationOveriddenOn == DateTime.MinValue ? "-" : tabulation.RecommendationOveriddenOn.ToString(System.Configuration.ConfigurationSettings.AppSettings["datePatternBackend"]);
            lblRecommendationOverridingRemarks.Text = tabulation.RecommendationOverridingRemarks == null ? "-" : tabulation.RecommendationOverridingRemarks;

            List<TabulationRecommendation> TabulationRecommendationList = tabulation.TabulationRecommendations;
            for (int i = 0; i < TabulationRecommendationList.Count; i++) {

                DateTime recommandedDate = TabulationRecommendationList[i].RecommendedDate;
                if (recommandedDate != DateTime.MinValue) {
                    DateTime NewRecommendedDate = recommandedDate.AddMinutes(330);
                    TabulationRecommendationList[i].RecommendedDate = NewRecommendedDate;
                }

            }

            gvRecommenations.DataSource = TabulationRecommendationList;
        ...
        protected void btnViewApprovals_Click(object sender, EventArgs e) {

            int TabulationId = int.Parse(hdnTabulationId.Value);

            var isUploaded = committeeController.IsDocsUploaded(int.Parse(ViewState["BidId"].ToString()), TabulationId, "P");
            TabulationMaster tabulationMaster = tabulationMasterController.GetTabulationsByTabulationId(int.Parse(hdnTabulationId.Value));
            decimal netTotal = tabulationMaster.NetTotal;

            if (isUploaded > 0 || netTotal <= 1000000) {

                List<TabulationApproval> TabulationList = new JavaScriptSerializer().Deserialize<PrMasterV2>(ViewState["PrMaster"].ToString()).Bids.Find(b => b.BidId == int.Parse(ViewState["BidId"].ToString())).Tabulations.Find(t => t.TabulationId == TabulationId).TabulationApprovals;
                for (int i = 0; i < TabulationList.Count; i++) {

                    DateTime recommandedDate = TabulationList[i].ApprovedDate;
                    if (recommandedDate != DateTime.MinValue) {
                        DateTime NewRecommendedDate = recommandedDate.AddMinutes(330);
                        TabulationList[i].ApprovedDate = NewRecommendedDate;
                    }

                }


                gvApproval.DataSource = TabulationList;
                gvApproval.DataBind();
                ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "none", "<script>    $(document).ready(function () { $('#mdlApprovals').modal('show'); });   </script>", false);
            }
            else {
        ...
        protected void btnReject_Click(object sender, EventArgs e) {
            ...
            int result = tabulationApprovalController.RejectAtApproval(int.Parse(hdnTabulationId.Value), int.Parse(Session["UserId"].ToString()), int.Parse(Session["DesignationId"].ToString()), hdnRejectRemarks.Value);
            TabulationMaster Tabulation = tabulationMasterController.GetTabulationsByTabulationId(int.Parse(hdnTabulationId.Value));
            List<BiddingItem> Biddinglist = ControllerFactory.CreateBiddingController().GetPrdIdByBidId(int.Parse(ViewState["BidId"].ToString()));
            ...
            if (Tabulation.IsApproved == 2) {
                for (int i = 0; i < Biddinglist.Count; i++) {
                    int prdId = Biddinglist[i].PrdId;
                    ControllerFactory.CreatePR_DetailController().UpdatePRStatus(int.Parse(HttpContext.Current.Session["UserId"].ToString()), prdId, "QTATION_APRVL_RJCTED");
            ...
            if (Tabulation.IsApproved == 1) {
                for (int i = 0; i < Biddinglist.Count; i++) {
                    int prdId = Biddinglist[i].PrdId;
                    ControllerFactory.CreatePR_DetailController().UpdatePRStatus(int.Parse(HttpContext.Current.Session["UserId"].ToString()), prdId, "CRTPO");

                }
            }
            for (int i = 0; i < Biddinglist.Count; i++) {
                int prdId = Biddinglist[i].PrdId;
                ControllerFactory.CreatePR_DetailController().UpdateLogStatus(int.Parse(HttpContext.Current.Session["UserId"].ToString()), prdId, "QTATION_APRVL_RJCTED");
        ...
        protected void btnApprove_Click(object sender, EventArgs e) {
            ...
            int result = tabulationApprovalController.ApproveTabulation(int.Parse(hdnTabulationId.Value), int.Parse(Session["UserId"].ToString()), int.Parse(Session["DesignationId"].ToString()), hdnApprovalRemarks.Value);
            ...
            for (int i = 0; i < Biddinglist.Count; i++) {
                int prdId = Biddinglist[i].PrdId;
                ControllerFactory.CreatePR_DetailController().UpdateLogStatus(int.Parse(HttpContext.Current.Session["UserId"].ToString()), prdId, "QTATNAPRVD");
        ...
        protected void btnOverrideRecommendationApprove_Click(object sender, EventArgs e) {
            ...
            int result = tabulationApprovalController.OverrideAndApprove(int.Parse(hdnRecommendationId.Value), int.Parse(hdnTabulationId.Value), int.Parse(Session["UserId"].ToString()), int.Parse(Session["DesignationId"].ToString()), hdnApprovalRemarks.Value, 1);
            ...
            for (int i = 0; i < Biddinglist.Count; i++) {
                int prdId = Biddinglist[i].PrdId;
                ControllerFactory.CreatePR_DetailController().UpdateLogStatus(int.Parse(HttpContext.Current.Session["UserId"].ToString()), prdId, "OVRRDN_APP");
        ...
        protected void btnOverrideRecommendationReject_Click(object sender, EventArgs e) {
            ...
            int result = tabulationApprovalController.OverrideAndApprove(int.Parse(hdnRecommendationId.Value), int.Parse(hdnTabulationId.Value), int.Parse(Session["UserId"].ToString()), int.Parse(Session["DesignationId"].ToString()), hdnRejectRemarks.Value, 2);
        ...
        protected void btnClone_Click(object sender, EventArgs e) {
            ...
            int clone = tabulationMasterController.CloneBid(BidId, int.Parse(ViewState["PrId"].ToString()), int.Parse(Session["UserId"].ToString()), openDays);
            if (clone > 0) {
                ...
                ScriptManager.RegisterClientScriptBlock(Updatepanel1, this.Updatepanel1.GetType(), "none", "<script>    $(document).ready(function () { swal({ type: 'success',title: 'SUCCESS',text:'Your work has been saved', showConfirmButton: false,timer: 1500}).then((result) => { window.location = 'ViewPrForQuotationRejectedApprovalNew.aspx' }); });   </script>", false);
        ...
        protected void btnTerminate_Click(object sender, EventArgs e) {
            ...
            int terminate = tabulationMasterController.TerminaeBid(BidId);
            if (terminate > 0) {
                for (int i = 0; i < bids.Count; i++) {
        ...
        protected void btnUpdatefinish_Click(object sender, EventArgs e) {
            ...
            if ((hdnSelectedChanged.Value != "" && hdnSelectedChanged.Value != "") || (hdnrejected.Value != null && hdnrejected.Value != "") || (hdnSlectedQutations.Value != "" && hdnSlectedQutations != null)) {
                ...
                if (IsUpdated > 0) {
                    var PrMaster = pr_MasterController.GetPrForQuotationConfirmation(int.Parse(ViewState["PrId"].ToString()), int.Parse(Session["CompanyId"].ToString()), new JavaScriptSerializer().Deserialize<List<int>>(ViewState["TabulationIds"].ToString()), int.Parse(Session["UserId"].ToString()), int.Parse(Session["DesignationId"].ToString()));
                    ViewState["PrMaster"] = new JavaScriptSerializer().Serialize(PrMaster);
                    LoadQuotations();
                    txtareaRemark.Text = "";
        ...
        protected void btnUpload_Click(object sender, EventArgs e) {
            if ((fileUpload1.PostedFile != null && fileUpload1.PostedFile.ContentLength > 0)) {
                var tabulationMaster = new JavaScriptSerializer().Deserialize<TabulationMaster>(ViewState["tabulationMaster"].ToString());
                int seq = 0;
                var isUploaded = committeeController.IsDocsUploaded(int.Parse(ViewState["BidId"].ToString()), tabulationMaster.TabulationId, "P");
                if (isUploaded > 0) {
                    seq = committeeController.GetUploedasequence(int.Parse(ViewState["BidId"].ToString()), tabulationMaster.TabulationId, "P");
                }
                ...
                for (int i = 0; i < uploadedFiles.Count; i++) {
                    ...
                    path = "~/ProcCommitteeDocs/" + imagename;
                    userPostedFile.SaveAs(Server.MapPath("ProcCommitteeDocs") + "\\" + imagename);
                    seq = seq + 1;
        ...
        protected void btnPrint1_Click(object sender, EventArgs e) {
          ...
          //  dt.Columns.Add("NbtAmount", typeof(string));
            ...
            //List<TabulationDetail> tabulationDetails = new JavaScriptSerializer().Deserialize<PrMasterV2>(ViewState["PrMaster"].ToString()).Bids.Find(b => b.BidId == int.Parse(ViewState["BidId"].ToString())).Tabulations.Find(q => q.TabulationId == tabulationMaster.TabulationId).TabulationDetails.Where(x => (x.TabulationId == tabulationMaster.TabulationId)).ToList();
            List<TabulationDetail> tabulationDetails = tabulationDetailController.GetTabulationDetail(tabulationMaster.TabulationId);
            ...
            for (int j = 0; j < tabulationDetails.Count; j++) {
                ...
                newRow["Description"] = tabulationDetails[j].Description;
            ...
            Response.AddHeader("content-disposition", "attachment;filename=" + FileName);
            string headerTable = @"<Table><tr><td  colspan='13'><center><h4>Quotation Tabulation For Bid No: B" + bid.BidCode + " </h4></center> </td></tr></Table> <hr>";
            Response.Write(headerTable);
            Response.Charset = "";
            Response.ContentType = "application/vnd.ms-excel";
            using (StringWriter sw = new StringWriter()) {
                ...
                for (int i = 0; i < gv.Rows.Count; i++) {
                    GridViewRow currentRow = gv.Rows[i];
                    if (currentRow.Cells[11].Text == "Selected") {
        ...
        protected void btnTechdocView_Click(object sender, EventArgs e) {
            try {
                ...
                ScriptManager.RegisterClientScriptBlock(Updatepanel1, this.Updatepanel1.GetType(), "none", "<script>    $(document).ready(function () {$('.modal-backdrop').remove(); $('#mdlviewdocsprocCommitee').modal('show');});   </script>", false);
            }
            catch (Exception ex) {
                throw ex;
            }
        ...
        protected void btnProcDocView_Click(object sender, EventArgs e) {
            try {
                hdoc.Value = "2";

                List<TecCommitteeFileUpload> bidingplandoc = committeeController.Gettechcommitteefiles(int.Parse(ViewState["BidId"].ToString()), int.Parse(hdnTabulationId.Value), "P");
                gvbddifiles.DataSource = bidingplandoc;
                gvbddifiles.DataBind();

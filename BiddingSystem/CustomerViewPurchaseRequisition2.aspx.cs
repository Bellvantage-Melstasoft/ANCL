...
{
    public partial class CustomerViewPurchaseRequisition2 : System.Web.UI.Page
    {
        ...
        private static string PRId = string.Empty;
        ...
        protected void btnApprove_Click(object sender, EventArgs e)
        {
            try
            {
                pr_MasterController.ConfirmOrDenyPRApproval(int.Parse(PRId), 1);
                ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "none", "<script>    $(document).ready(function () { swal({ type: 'success',title: 'Your work has been saved', showConfirmButton: false,timer: 1500}); window.location = 'CustomerPRViewForConfirmation.aspx'; });   </script>", false);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        protected void btnRejectPR_Click(object sender, EventArgs e)
        {
            try
            {
                pr_MasterController.ConfirmOrDenyPRApproval(int.Parse(PRId), 2);
                ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "none", "<script>    $(document).ready(function () { swal({ type: 'success',title: 'Your work has been saved', showConfirmButton: false,timer: 1500}); window.location = 'CustomerPRViewForConfirmation.aspx'; });   </script>", false);

                //Response.Redirect("CustomerPRViewForConfirmation.aspx", true);
            }
            catch (Exception ex)
            {
                throw ex;

...
{
    public partial class ApprovedPOInvoices : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Session["CompanyId"] != null && Session["UserId"].ToString() != null)
            {
                ...
                ((BiddingAdmin)Page.Master).subTabValue = "ApprovedPOInvoices.aspx";
                ((BiddingAdmin)Page.Master).subTabId = "ApprovedPOInvoicesLink";
            ...
            if (!IsPostBack)
            {
                if (int.Parse(Session["UserId"].ToString()) != 0)
                {
                    try
                    {
                        ...
                        pOMasterListByDepartmentid = pOMasterController.GetPosForInvoice(int.Parse(Session["CompanyId"].ToString()), int.Parse(Session["UserId"].ToString()));
        ...
        protected void btnView_Click(object sender, EventArgs e) {
            try {
                ...
                Response.Redirect("AddInvoice.aspx?PoId=" + PoId + "&Redirect=ApprovedPOInvoices");
            }
            catch (Exception ex) {
                throw ex;
            }
